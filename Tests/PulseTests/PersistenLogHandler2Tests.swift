...
final class PersistentLogHandler2Tests: XCTestCase {
    var tempDir: TempDirectory!
    var storeURL: URL!
    var store: LoggerMessageStore2!
    var sut: PersistentLogHandler2!
    ...
    override func setUp() {
        ...
        sut = PersistentLogHandler2(label: "test-hanlder", store: store, makeCurrentDate: { Date() })
    }
    ...
    func testItPersistsLoggedMessages() throws {
        ...
        LoggingSystem.bootstrap {
            MultiplexLogHandler([
                PersistentLogHandler2(label: $0, store: self.store, makeCurrentDate: { date }),
